...
- **Transactions** — log expenses and income with an amount, category, date,
  and note.
- **Categories** — a sensible default set, fully editable; add, archive, or
  recolor your own.
- **Budgets** — set a weekly or monthly spending limit, either overall or
  per category, with live progress.
- **Dashboard** — this month's spending broken down by category (donut
  chart), recent activity, and budget progress at a glance.
- **Google Drive backup** — one Google sign-in, then "Back up now" /
  "Restore from Drive" on demand. The app only ever touches files it
  creates itself in a "Buget Backups" folder — never the rest of your
  Drive.
...
3. **Configure the OAuth consent screen**: APIs & Services → OAuth consent
   screen.
   - User type: **External**.
   - Fill in the app name (e.g. "Buget"), your email, etc.
...
4. **Create three OAuth client IDs** (APIs & Services → Credentials →
   ...
   - **Web application** — no redirect URIs needed. Copy its **Client ID**;
     ...
     mistake.)
   - **Android** — Package name: `com.buget.app`. SHA-1 certificate
   ...
   - **iOS** — Bundle ID: `com.buget.app`. Copy its **Client ID**, then
...
**Backup** writes your whole local database as one JSON file to a "Buget
...
- Sync is manual (a button), not automatic background sync — deliberate,
  to keep behavior predictable and avoid surprise mobile-data usage, but a
  "sync on app open when online" toggle would be a reasonable addition.
- No CSV/PDF export yet.
- No recurring transactions (e.g. auto-log rent every month).
- No multi-currency conversion — each transaction just stores whatever
  currency was selected at the time; there's no exchange-rate rollup.
- No passcode/biometric lock on the app itself.
